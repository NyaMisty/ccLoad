...
For native Claude Code traffic that reaches an Anthropic Messages upstream, ccLoad provides persistent soft affinity for valid UUID sessions. It prefers the inbound `X-Claude-Code-Session-Id` and falls back to the inbound `metadata.user_id.session_id`, then prefers the most recently successful actual upstream key within the same inbound API-token scope. Target kind plus key hash identify the target; API-key row and channel IDs are lookup hints only, and no URL is bound. Affinity only probes a key that remains allowed, enabled, and outside cooldown; an unavailable or failed key returns routing to its unchanged normal channel order. The first fallback key that succeeds immediately becomes the new affinity target, so recovery of an older key does not pull the session back. The binding is retained for one hour after its latest successful hit and is stored in `claude_session_affinities` without plaintext tokens, sessions, or keys. For ordinary API-key upstreams, the Session ID sent in both the header and body is stably derived from the actual selected key plus the inbound Session ID. Z.ai Coding Plan is the OAuth exception: its actual `id.secret` key participates in the same affinity policy, while its upstream Session ID uses a separate ZCode derivation domain and is written to both `metadata.user_id.session_id` and `x-session-id`. A 401 re-resolution immediately moves affinity to the replacement Coding Plan key. Every retry derives from the original inbound Session ID, never from the preceding attempt. Other OAuth channels do not participate.
...
- **Log Semantics**: Queue pressure applies backpressure instead of dropping entries, and a failed authoritative write retains the exact batch and retries with capped backoff until it commits. Each primary-replica log batch also has a distinct non-coalescing task and retries after failure; newer batches cannot replace older ones. Ten pending replica batches are retained before backpressure propagates to the log worker, bounding memory while the primary is unavailable. Retention cleanup and administrator-requested channel deletion remain explicit deletion policies.
...
- **Boundary**: This is a single-instance, single-writer design. The committed SQLite log remains authoritative if an in-memory primary-replica task is interrupted by process termination; there is no durable primary-sync outbox or multi-instance write coordination
