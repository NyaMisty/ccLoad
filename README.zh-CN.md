...
| 🚧 **单 Key 并发限制** | 渠道内每个 Key 的同时在飞请求上限 | 0=不限，满载 Key 自动跳过 |
...
> **并发限制说明**：`max_concurrency` 是渠道内每个 API Key 的同时在飞请求上限；`0` 表示不限制。槽位从发起上游请求前占用，到响应体关闭后释放，流式请求会占用到流结束。满载 Key 会被跳过且不消耗上游 Key 重试预算；检查不到有空闲槽位的 Key 时才跳过渠道。计数保存在当前进程内，多实例部署时各实例独立统计。
...
- ✅ **单 Key 并发限制**：渠道内每个 Key 的同时在飞请求数上限，`0` 表示无限制，满载 Key 自动跳过
