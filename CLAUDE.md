...
- **范围与身份**:仅作用于客户端和实际上游均为 Anthropic Messages 的 `POST /v1/messages`/`POST /messages`;身份键是入站 Token 哈希 + 合法 UUID Claude session。入站 session 优先取 `X-Claude-Code-Session-Id`,缺失或非法时回退 `metadata.user_id.session_id`,亲和始终使用该入站值。API Key 上游的 wire session 再由「实际选中的 Key + 入站 session」稳定派生,同步写入 Header 与 Body;因此同 session 同 Key 稳定、换 Key 换 wire session、切回原 Key 恢复原值。OAuth 没有 Key,保持原 session;`device_id` 不是 session,仍按本次实际选中的上游 Key 独立派生
- **目标与软语义**:亲和目标只能是上游 API Key,不绑定渠道或 URL;`api_key_id` 标识 Key 行,`channel_id + key_index` 只负责路由定位,实际命中前还必须校验 Key 哈希。成本、可用时段、Token 渠道权限、禁用和冷却先过滤,目标 Key 不健康时回到未改动的普通渠道顺序。首个成功的 fallback Key 立即成为新亲和目标,旧 Key 恢复后不会把 session 拉回去;OAuth 没有 API Key,不建立此亲和
