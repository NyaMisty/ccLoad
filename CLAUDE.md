...
- **范围与身份**:仅作用于客户端和实际上游均为 Anthropic Messages 的 `POST /v1/messages`/`POST /messages`;身份键是入站 Token 哈希 + 合法 UUID Claude session。入站 session 优先取 `X-Claude-Code-Session-Id`,缺失或非法时回退 `metadata.user_id.session_id`,亲和始终使用该入站值。普通 API Key 上游的 wire session 由「实际选中的 Key + 入站 session」稳定派生并同步写入 Header/Body;Z.ai Coding Plan 使用独立域 `"ccload:zai:session\0" + Coding Plan Key + "\0" + 入站 session` 派生并同步写入 `metadata.user_id.session_id`/`x-session-id`。每次重试都从原始入站 session 派生,禁止级联使用上次 attempt 的 wire session
- **目标与软语义**:亲和目标是普通上游 API Key 或 Z.ai 实际 Coding Plan Key,不绑定渠道或 URL;`target_kind + api_key_hash` 是权威身份,`api_key_id + channel_id` 只作查找提示。成本、可用时段、Token 渠道权限、禁用和冷却先过滤,目标 Key 不健康时回到未改动的普通渠道顺序。首个成功的 fallback Key 立即成为新亲和目标,旧 Key 恢复后不会把 session 拉回去;ZCode 401 重解析成功后立即改绑新 Coding Plan Key,其他 OAuth 没有实际 API Key,不建立此亲和
- **生命周期**:目标自最后一次成功命中起保留 1h,到期后由下一次成功 Key 重新绑定。只把 session/Token 组合摘要、Key 摘要、目标类型和定位提示写入 `claude_session_affinities`,不保存明文 Token、session 或 Key;纯数据库模式写当前数据库,混合模式写权威 SQLite,进程重启后仍可恢复。该表的目标结构变更不做启动迁移,升级旧部署前必须显式删除旧表并由新版本重建
...
- **Claude Code CLI 指纹**(`anthropic_wire.go`,无开关):套不套 CLI 指纹只由 `isAnthropicClaudeCodeMessagesRequest`(anthropic 上游 + `/v1/messages` 且非 Z.ai Coding Plan)决定,**与凭证形态无关**——OAuth、第一方 API Key、第三方 Anthropic 网关一律走同一条 `finalizeAnthropicClaudeCodeMessagesBody` + `applyAnthropicClaudeCodeHeaders`,认证差异只落在认证头上(OAuth `Authorization: Bearer`,API Key 走 `applyAnthropicAPIKeyAuth`:官方 origin 只发 `x-api-key`,第三方网关两种都发)。API Key 渠道没有真实 device/account,由 `synthesizeAnthropicAPIKeyCredential` 从最终选中的上游 Key 稳定派生;每次实际转发都用 `bindAnthropicAPIKeyDeviceID` 覆盖调用方 `metadata.user_id.device_id`,并用 `anthropicUpstreamSessionID` 从最终 Key + 入站 session 派生 wire session,同步覆盖 `metadata.user_id.session_id` 与 `X-Claude-Code-Session-Id`,所以同 Key 同设备、同 session 同 Key 同 wire session,换 Key 两者都换,包括原生直通、Haiku 辅助请求和 Key 故障切换。亲和识别必须继续读派生前的入站 session,重试也必须从原始 body/header 派生,禁止拿上一次 attempt 的 wire session 级联派生。**别拆成「OAuth 版」和「API Key 版」两套形态**:body 形态与 `anthropic-beta` 必须同源,拆开就会出现 body 用了 `cache_control.ttl=1h` 而 header 少了 `extended-cache-ttl-2025-04-11` 的 400。三类请求例外直通(保留其余 body,覆盖 API Key 身份并重签 CCH):`nativeAnthropicHaikuHelperShape` 命中的内部 Haiku 辅助请求、`isNativeAnthropicClaudeCodeRequest` 命中的原生 Claude Code 请求(判定含 header 与 body 的 session id 等式)、以及 Z.ai(使用自己的设备契约,不做 API Key 覆盖)。副作用:该路径清空全部下游 header 后重建,所以 `buildProxyRequest`/管理测试在重建之后**再跑一次** `applyHeaderRules`——渠道自定义 header 规则必须最终生效,认证头仍由 `authHeaderBlacklist` 拦下,规则可以改写 CLI 身份头,但 API Key wire session 会在规则后从 Body 最终同步,不可覆盖(重跑对 override/remove 幂等,append 也不重复,因为前一次的产物已被清空);同理 `applyHeaderRules` 会先把规则名对齐到请求里已存在的同名键,否则 canonical 化会让 `anthropic-beta` 这类小写指纹头以两种大小写并存。CCH 无条件重签(`finalizeAnthropicCCH` 对无 billing header 的 body 是 no-op),别加第二个谓词
