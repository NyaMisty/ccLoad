...
func TestKeyConcurrencyLimiterWaitIsScopedToKey(t *testing.T) {
	t.Parallel()

	limiter := newChannelConcurrencyLimiter()
	releaseA, _, _, ok := limiter.acquire(7, "key-a", 1)
	if !ok {
		t.Fatal("pre-acquire key-a failed")
	}
	defer releaseA()

	releaseB, err := limiter.acquireContext(context.Background(), 7, "key-b", 1)
	if err != nil {
		t.Fatalf("key-b should not wait for key-a: %v", err)
	}
	releaseB()

	ctx, cancel := context.WithTimeout(context.Background(), 50*time.Millisecond)
	defer cancel()
	if _, err := limiter.acquireContext(ctx, 7, "key-a", 1); !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("key-a wait error=%v, want context deadline exceeded", err)
	}
}
...
func TestKeyConcurrencyExhaustedErrorIncludesScanCounts(t *testing.T) {
	t.Parallel()
